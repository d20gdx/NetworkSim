...
public class Runner {
    ...
    /**
     * Main method to execute LPWAN Network Contention Simulator Run with gateway
     * name parameter to switch targets
     * 
    ...
    public static void main(String args[]) {
        ...
        for (int run = props.getMinNodesInRun(); run <= props.getMaxNodesInRun(); run += props
                .getIncreaseRunStepSize()) {

            props.setNetworkSwamped(false);
            props.setStartTime(System.currentTimeMillis());
            props.setNoNodesInRun(run);

            System.out.println("Starting run gateway " + props.getTargetGateway() + " for " + run
                    + " nodes at spreading factor " + props.getSpreadingFactor());
